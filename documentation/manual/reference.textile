...
PlayMorphia specific tag. Lookup message string.
...
PlayMorphia specific tag. Reverse lookup URL.
...
PlayMorphia specific tag. Absolute reverse lookup URL.
...
PlayMorphia specific tag. This is a clone of the "authenticityToken":http://www.playframework.org/documentation/1.2.4/tags#authenticityToken tag in Play's groovy template.
...
PlayMorphia specific tag. Use @authenticityTokenValue@ when you want to put the authenticity token value inside a javascript code in your view:
...
PlayMorphia specific tag. Outputs the validation error message, if present, for the field specified by the tag parameter.
...
PlayMorphia specific tag. Outputs the text @hasError@ if there is a validation error for the field specified by the tag parameter. This is useful for setting a CSS class for input fields with errors:
