...
** Support ".cache(...)" extension to tag invocation
** Support <code>@for(int i = 0; i < 100; ++i)</code> loop style
** <code>@return()</code>: abort current template executing process and return to caller
** <code>@break()</code>: break the current loop: <code>@for(...) {... @break() ...}</code>
* Small enhancements
