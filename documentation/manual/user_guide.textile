...
p(note). Unlike groovy engine, there is no special place for tag file storage. Basically every template can be invoked as a tag.
...
h5. <a name="call_tag_relative_path">Call tag using relative and import path</a>
...
p(note). Because controller action invocation are treated as tag invocation, therefore cache tag invocation result also apply to controller action invocation. Please refer to "Cache tag invocation result":user_guide#tag_invocation_cache for details
