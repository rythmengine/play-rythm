...
p(note). If you switch tag invocation to <code>@include</code> and found something has changed even there is no parameter call it must due to you have <code>@extended</code> layout template in the tag template. <code>include</code> will NOT be able to include the layout template parts
