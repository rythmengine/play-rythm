...
Templates are stored in @app/rythm@ directory.
...
p(note). The "tag" in Rythm is called "macro" in Velocity and WebMacro, however Rythm tag facility are more powerful than macro in Velocity and FreeMarker. See "Tag invocation":#invoke_tag for more information.
...
p(note). You cannot pass argument by name when invoking inline tag. Use pass by position instead:
