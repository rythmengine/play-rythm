...
p(note). Unlike <code>@play.cache.CacheFor</code>, with the new <code>@Cache4</code>, if the action result get from cache, then the @Before and @After filters will not be executed. Also it might break the state of other plugins if they were using @beforeActionInvocation()@ and @onActionInvocationResult()@ pair
