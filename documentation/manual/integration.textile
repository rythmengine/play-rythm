...
# -You must use full qualified method name (including package name, class name) when you invoke controller action method- No longer needed since v1.0.0
...
You can set @useSessionData@ attribute to @true@

p(note). Unlike <code>@play.cache.CacheFor</code>, with the new <code>@Cache4</code>, if the action result get from cache, then the @Before and @After filters will not be executed. Also it might break the state of other plugins if they were using @beforeActionInvocation()@ and @onActionInvocationResult()@ pair. Rule of thumb: if you expect each request be handled through some filters, e.g. secure gate, DO NOT use <code>@Cache4</code>
